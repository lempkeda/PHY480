//  MODIFIED SLIGHTLY FOR USE IN Final.cpp
//  DAVE LEMPKE
//  PHY 480 4/29/21
...
const int NMAX=100;   // maximum of NMAX coupled equations
	...
	vector<vector<parameters>> &vec)
{
  ...
  for (int i = 0; i < N; i++)
    {
      k2[i] = h * f (t + h / 2., y1, i, vec);
      y2[i] = y[i] + k2[i] / 2.;	// argument for k3 
    }

  for (int i = 0; i < N; i++)
    {
      k3[i] = h * f (t + h / 2., y2, i, vec);
      y3[i] = y[i] + k3[i];	// argument for k4 
    }

  for (int i = 0; i < N; i++)
    {
      k4[i] = h * f (t + h, y3, i, vec);
    }
